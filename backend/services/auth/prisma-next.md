...
Your data contract is the heart of your application. It lives at [`config/prisma/auth.prisma`](config/prisma/auth.prisma) and describes your models:
...
export default definePrismaConfig({
  orm: ormConfig({
    contract: './config/prisma/auth.prisma',
    db: {
...
| File | Purpose |
|---|---|
| [`config/prisma/auth.prisma`](config/prisma/auth.prisma) | Your data contract — define your models here |
| [`prisma.config.ts`](prisma.config.ts) | CLI configuration |
| [`config/prisma/db.ts`](config/prisma/db.ts) | Database client — `import { db } from './config/prisma/db'` |
| `config/prisma/contract.json` | Compiled contract (generated) |
| `config/prisma/contract.d.ts` | Contract types (generated) |

### Workflow

1. Edit [`config/prisma/auth.prisma`](config/prisma/auth.prisma) to add or change models.
